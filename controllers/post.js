...
exports.createPost = async (req, res) => {
   ...
   if (isAlreadyExists) 
   return res.status(401).json({error: 'Plese use unique slug!'});
...
if (!title.trim()) 
    return res.status(401).json({error: "Search term not found!"});
...
exports.getRelatedPosts = async (req, res) => { 
 ...
 return res.status(404).json({error: "Post not found!"});
    ...
    const getRelatedPosts = await Post.find({
        tags: { $in: [...post.tags] },
        _id: { $ne: post._id },
    })
    .sort({createdAt: -1}) .limit(5)
       ...
       res.json({ 
        posts: getRelatedPosts.map((post) => ({
            id: post._id,
            title: post.title,
            meta: post.meta,
            slug: post.slug, 
